...
import { replayToStory, ProjectStyles, type PlayState, type InteractionStep, type ResolveAsset } from '@kiny/player'
...
export function App() {
  ...
  if (phase.kind === 'ready') {
    const { story, assetBase, projectCss, title, version, program, start, seed } = phase.loaded
    ...
    const fresh = (notice: string | null): PlayingPhase => {
      ...
      return { kind: 'playing', story: freshStory, resolveAsset, projectCss, program, start, progKey, seed: s, initialSeq: [], notice }
    }
    ...
    const onStart = () => {
      ...
      if (saved === null) {
        ...
        setPhase({ kind: 'playing', story, resolveAsset, projectCss, program, start, progKey, seed, initialSeq: [], notice: null })
        return
      }
      ...
      setPhase({
        kind: 'playing', story: r.story, resolveAsset, projectCss, program, start, progKey,
  ...
  function freshFrom(p: PlayingPhase): PlayingPhase {
    ...
    return { kind: 'playing', story, resolveAsset: p.resolveAsset, projectCss: p.projectCss, program: p.program, start: p.start, progKey: p.progKey, seed: s, initialSeq: [], notice: null }
  }
}
