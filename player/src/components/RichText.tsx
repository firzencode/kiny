 ...
 * （与打字中的 RevealingLine 共用同一样式映射，两阶段 DOM 样式一致，Q2）+ `kin-` 前缀作品 class；
 * 无样式无 class 的纯文本不包裹。 */
function renderSpan(span: RichSpan, key: number): ReactNode {
  if (isBreak(span)) return <br key={key} />
  const style = spanStyle(span)
  const cls = spanClassName(span.classes)
  if (Object.keys(style).length === 0 && cls === undefined) return <Fragment key={key}>{span.text}</Fragment>
  return <span key={key} className={cls} style={style}>{span.text}</span>
}
