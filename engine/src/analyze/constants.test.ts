...
describe('constants', () => {
  it('保留字 8 个、命令 15 个', () => {
    ...
    expect(COMMAND_NAMES.size).toBe(15)
    expect(JS_GLOBALS.has('Math')).toBe(true)
    expect(COMMAND_NAMES.has('bg_show')).toBe(true)
    expect(COMMAND_NAMES.has('sfx')).toBe(true)
    expect(COMMAND_NAMES.has('clear')).toBe(true)
    expect(COMMAND_NAMES.has('step_mode')).toBe(true)
    ...
    expect(COMMAND_NAMES.has('img')).toBe(true)
    expect(COMMAND_NAMES.has('divider')).toBe(true)
  })
