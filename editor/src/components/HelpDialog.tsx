...
const SECTIONS: Section[] = [
  ...
  { cat: '宿主', id: 'divider', gl: '@—', zh: '正文分割线', en: 'Divider',
    desc: '<code>@divider([类名])</code> 在正文流里插一条分割线，用于场景转换与幕间。与插图同形态——是<b>正文流里的一条内容</b>：随正文滚动、留在阅读历史里、<code>"line"</code> 模式下独占一次点击、<code>@clear()</code> 时随正文一并清除。<b>默认外观</b>是一条居中短线，线色随作品主题走；分割线本身即装饰，故播放层给的是完整外观而非「安全底线」。要改成满宽线、图案分隔符等形态，覆盖基线类名 <code>.kin-divider</code>，或传一个自己的<b>类名</b>（可选，规则同 <code>&lt;class=名&gt;</code>，渲染时加 <code>kin-</code> 前缀；参数可用变量，类名非法只忽略类名、分割线照常出现）。分割线是<b>块级</b>内容、只用于正文流——选项文本与 <code>@panel</code> 模板是行内富文本的地盘，那里要断行用 <code>&lt;br&gt;</code>。',
    code: '第一幕结束。\n@divider()\n第二幕开始。\n\n@divider("幕间")   // 交给 .kin-幕间 自定义外观' },
...
const THEME_CLASSES: { name: string; use: string }[] = [
  ...
  { name: '.kin-illustration', use: '正文插图（@img）——每张图恒带，作者类名只作追加' },
  { name: '.kin-divider', use: '正文分割线（@divider）——每条恒带，默认居中短线，改宽度 / 样式在此' },
