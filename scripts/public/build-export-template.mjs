...
/**
 * 生成 editor「导出独立网页」用的 viewer 空壳模板，并作为 Tauri resource 落到 editor。
 *
 * 流程（与 viewer 同源，改了 viewer 重跑即同步）：
 *  1. `viewer` 跑 `build:template`（`vite build --mode template`）——单文件内联 JS bundle、
 *     相对 base、不打包 demo，产物 `viewer/dist-template/index.html`。
 *  2. 在 `<head>` 注入 `window.__KINY_PROJECT__` 占位（先于 body 末尾的 bundle 执行）：
 *     导出时 Rust 把占位字符串 `"__KINY_PROJECT_DATA__"` 替换为实际内联数据；未注入则保持
 *     字符串、viewer 的 loadStory 识别为非内联、回退 fetch（空壳无 demo，仅占位安全）。
 *  3. 写到 `editor/src-tauri/resources/export-template/index.html`（gitignored 构建产物）。
 *
 * 前置：engine/player 已 `build:core`（viewer 依赖其 dist）。
...
const viewer = join(repoRoot, 'viewer')
const distIndex = join(viewer, 'dist-template', 'index.html')
...
console.log('[build-export-template] 构建 viewer 空壳模板（vite build --mode template）...')
execSync('npm run build:template', { cwd: viewer, stdio: 'inherit' })
