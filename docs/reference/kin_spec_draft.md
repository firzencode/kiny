...
| `<br>` | 显式换行 | 自闭合，无闭合标签 |
| `<pause>` | 句中停顿 | 自闭合，无值；打字揭示到此停住，读者点击才续显后半句 |
...
**`<pause>` 句中点击续显**——把悬念的节奏交给作者：

```
凶手就是…<pause>你自己！
```

前半句显示后打字停住、推进提示三角亮起，读者点击才续显后半句。它是**纯呈现层的分段揭示**：整行仍是一个文本事件、一条记录，不是引擎暂停点，不产生存档语义，读档与重放整行直显。

- 行内任意位置、可多个；可位于 `<b>` / `<color>` 等样式范围内，不影响样式作用域；与行末 `<>` 粘连正交（拼行后标记位置保留）。
- 行首标记 = 先等一次点击再出文字；连续 `<pause><pause>` 合并为一次；行尾标记忽略（行尾本就是行边界）。
- 停顿**不可被打字跳过穿透**：打字中点击只让当前段立即显示、仍停在标记处，再点一次才续下一段。
- 两种推进模式一致——`flow` 的自动续步只发生在**整行**揭示完之后。读者开启「减弱动态效果」时每段瞬显，但分段停顿保留（停顿是叙事节奏，不是动效）。
- 自闭合写法 `<pause/>` 等价；强制字面写法用 `\<pause>`；`<pause=毫秒>`（定时自动续显）是保留语法位，当前报错。
- 只对**正文行**有效：选项列表里的标记不产生停顿（选项是一次性呈现的按钮，没有揭示过程）；选中后进入正文的那部分文字照常生效。
- 停顿由读者点击解除。**定时**的停顿是行与行、命令与命令之间的演出编排，用 `@sleep`（§11.1）。
...
`@clear` 清的是**已显示正文**，与场景（背景 `@bg_*`、音乐 `@bgm_*`）相互独立——清屏后背景与 BGM 照旧。命令是硬边界，故同一次推进里 `@clear` 之前的文字先显示、`@clear` 清空、之后的文字进入清空后的新画面，天然实现「先清屏再显示新文」。
...
它是命令，而命令是硬边界：`@sleep` 之前的文字先成行显示、之后的文字进入新的一行，glue `<>` 也跨不过命令。**句中停顿**（前半句停住、后半句续显在同一行）用行内标记 `<pause>`（§3.6）。
...
- 模板是字符串，支持 `{表达式}` 插值与全部富文本标签（`<pause>` 在面板中无揭示流程，忽略）。**登记时不求值**，存模板本体；再次对同一槽 `@panel` = 整体替换。
