...
│   ├── city.kin
│   └── ending.kin
├── theme.css          # 可选：作品主题（项目内任意位置的 .css 都自动加载）
└── assets/            # 可选：图、音、字体
    ├── images/
    ├── audio/
    └── fonts/
...
作品换肤有两级：覆盖播放层的 CSS 变量（`--kiny-page-bg` / `--kiny-text` / `--kiny-prose-font` 等，一行 css 即可换字体或配色），或直接以 `.player` 为根书写选择器深度定制。正文里 `<font=名>` 切换句内字体、`<class=名>` 挂语义类名（详见 §3.6）。

**js 不自动加载**：项目内的 `.js` 可以存放与编辑，但不会被执行。
...
- **取值受限**：`color` 只接受上述颜色形式、`size` 只接受正数倍数、`font` 只接受单个族名（Unicode 字母数字与空格 `. _ -`）、`class` 更严（不含空格与点），**不接受任意 CSS**（杜绝经样式注入）。
